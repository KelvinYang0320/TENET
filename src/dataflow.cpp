...
Dataflow::Dataflow(Statement &&st, PEArray &&pe, Mapping &&mp) : _st(move(st)), _pe(move(pe)), _mp(move(mp)) {}
...
isl_union_map *Dataflow::GetAccess(string tensor_name, AccessType type)
{
...
isl_union_map *Dataflow::MapTimeToPrev(unsigned distance, bool is_range)
{
    ...
    else  // querying time within a given distance
    {
        ...
        isl_union_map *neighbor = isl_union_set_lex_gt_union_set(GetTimeDomain(), GetTimeDomain());
 ...
 */
isl_union_map *Dataflow::MapSpaceTimeToNeighbor(unsigned space_distance, bool space_is_range, unsigned time_distance,
                                                bool time_is_range, bool include_self)
{
...
*/
int Dataflow::GetUniqueVolume(string tensor_name, AccessType type, isl_union_map *space_time_to_neighbor)
{
    isl_union_map *access          = MapSpaceTimeToAccess(tensor_name, type);
    isl_union_map *neighbor_access = isl_union_map_apply_range(space_time_to_neighbor, isl_union_map_copy(access));
...
*/
double Dataflow::GetReuseFactor(string tensor_name, AccessType type, isl_union_map *space_time_to_neighbor)
{
...
{
    long long ingress_volume = GetUniqueVolume(tensor_name, AccessType::READ, space_time_to_neighbor) * BIT_PER_ITEM;
...
{
    long long egress_volume = GetUniqueVolume(tensor_name, AccessType::WRITE, space_time_to_neighbor) * BIT_PER_ITEM;
...
int Dataflow::GetL2Read(string tensor_name, AccessType type, isl_union_map *space_time_to_neighbor)
{
    if (type == AccessType::READ || type == AccessType::WRITE)
...
int Dataflow::GetL2Write(string tensor_name, AccessType type, isl_union_map *space_time_to_neighbor)
{
    if (type == AccessType::READ || type == AccessType::WRITE)
...
unsigned long long Dataflow::GetEnergy(isl_union_map *space_time_to_neighbor)
{
    ...
    for (auto &iter : input) {
        ...
        energy += l2_multiplier * GetL2Read(iter, AccessType::READ, isl_union_map_copy(space_time_to_neighbor));
        energy += l2_multiplier * GetL2Write(iter, AccessType::READ, isl_union_map_copy(space_time_to_neighbor));
    }
    for (auto &iter : output) {
        energy += l1_multiplier * GetL1Read(iter, AccessType::WRITE);
        energy += l1_multiplier * GetL1Write(iter, AccessType::WRITE);
        energy += l2_multiplier * GetL2Read(iter, AccessType::WRITE, isl_union_map_copy(space_time_to_neighbor));
        energy += l2_multiplier * GetL2Write(iter, AccessType::WRITE, isl_union_map_copy(space_time_to_neighbor));
    }
    isl_union_map_free(space_time_to_neighbor);
    return energy;
}

Dataflow Dataflow::copy() const
{
